...
description: Operational reference for Fly Sprite-hosted Claude Code as a third cloud-delegation target. Use when running or debugging Claude Code inside a Sprite VM — the `sprite` CLI surface, threading auth for unattended runs, what the VM can reach (hex.pm, Elixir/Mix, Tidewave, a public HTTPS URL), sleep/wake and state persistence, capping cost per run, and the known orchestration gaps around completion signalling. Sibling of `cloud-agent-environments`, `linear-queue`, `agent-dispatch`, `agent-pr-review`, `flow-review`.
...
- Commit / push / PR-create inside a sprite are allowed by default, same as anywhere else; only the irreversible/outward gates remain (`delegation-rules.md` § "DON'T AUTO-MERGE PRS" / cloud-agent branches).
