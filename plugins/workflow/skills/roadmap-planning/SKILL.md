...
**Scope:** applies to **review-surface findings** (pre-commit `/code-review`; post-merge audit per `verification-policy.md`). Discoveries during `/research`, `/plan`, or implementation follow the discovery-capture rules (file via `rmap new`) — not this floor.
